...
const testimonials = [
  { name: "أستاذ بن علي محمد", role: "محامي - الجزائر العاصمة", text: "المنصة غيرت طريقة إدارتي لمكتبي بشكل كامل. أنجز المهام بسرعة أكبر." },
    ...
    <div className="min-h-screen bg-surface" suppressHydrationWarning>
      ...
      <section className="relative bg-gradient-to-bl from-primary via-primary-light to-primary-dark text-white overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className={`absolute top-20 right-20 w-72 h-72 bg-secondary rounded-full blur-3xl animate-hero-blob ${showSplash ? "opacity-0" : ""}`} />
          <div className={`absolute bottom-20 left-20 w-96 h-96 bg-accent rounded-full blur-3xl animate-hero-blob ${showSplash ? "opacity-0" : ""}`} style={{ animationDelay: "0.3s" }} />
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 relative z-10">
          <div className="max-w-3xl">
            <h1 className={`text-4xl md:text-6xl font-bold leading-tight mb-6 ${!showSplash ? "animate-hero-heading" : "opacity-0"}`}>
              إدارة مكتبك القانوني <span className="text-secondary">بذكاء واحترافية</span>
            </h1>
            <p className={`text-xl text-gray-200 mb-8 leading-relaxed ${!showSplash ? "animate-hero-text" : "opacity-0"}`}>
            ...
            <div className={`flex flex-wrap gap-4 ${!showSplash ? "animate-hero-buttons" : "opacity-0"}`}>
            ...
            <div className={`mt-8 flex items-center gap-6 text-sm text-gray-300 ${!showSplash ? "animate-hero-badges" : "opacity-0"}`}>
